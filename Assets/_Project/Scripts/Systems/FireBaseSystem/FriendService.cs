...
public class FriendService
{
    ...
    private EventHandler<ValueChangedEventArgs> friendsDataChangedHandler;
    public event Action<List<FriendData>> OnFriendsListChanged;

    public void StartListeningForFriendsChanges()
    {
        if (!ValidateUserAndDatabase(null)) return;

        StopListeningForFriendsChanges();

        friendsDataChangedHandler = (object sender, ValueChangedEventArgs args) =>
        {
            ...
            if (args.Snapshot.Exists)
            {
                ...
                OnFriendsListChanged?.Invoke(friendsList);
            }
            else
            {
                OnFriendsListChanged?.Invoke(new List<FriendData>());
            }
        };

        DatabaseReference friendsRef = databaseRef.Child("users").Child(user.UserId).Child("Friends");
        friendsRef.ValueChanged += friendsDataChangedHandler;
    }

    public void StopListeningForFriendsChanges()
    {
        if (friendsDataChangedHandler != null && user != null && databaseRef != null)
        {
            DatabaseReference friendsRef = databaseRef.Child("users").Child(user.UserId).Child("Friends");
            friendsRef.ValueChanged -= friendsDataChangedHandler;
            friendsDataChangedHandler = null;
        }
    }
    ...
    public void SendFriendRequest(string friendName, Action<bool, string> callback = null)
    {
        ...
        userDataService.FindUserByDisplayName(friendName, (friendData, error) =>
        {
            ...
            var friendRequest = new FriendData(friendData);
    ...
    private void GetFriendsList(Action<List<FriendData>, string> callback)
    {
        if (user == null)
        {
    ...
    public void GetFriendByStatus(FriendStatus status, Action<List<FriendData>, string> callback)
    {
        GetFriendsList((allFriends, error) =>
        {
            ...
            var filteredFriends = allFriends.FindAll(friend => friend.FriendStatusEnum == status);
